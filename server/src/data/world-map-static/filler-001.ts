...
"arena_quarter_fill_24_10": {
    ...
    "name": "西南器械欄",
    "zone": "arena_quarter",
    "description": "西南器械欄貼著訓練沙地外側，北面能看見木槍架與沙袋，東側被裁判繩圈隔開。這裡只是競技場平面上的封閉邊角，用來標出訓練場邊界，玩家不能從此穿入看台或後勤區。西南器械欄周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"arena_quarter_fill_24_6": {
    ...
    "name": "北側檢查欄",
    "zone": "arena_quarter",
    "description": "北側檢查欄位於武器寄放線外，南面連著競技區主路的喧鬧聲，東西兩側堆著封存木箱與鎖鏈。守衛把這段欄位作為不可通行邊界，提醒玩家改走正式入口與票券柱廊。北側檢查欄周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"arena_quarter_fill_25_11": {
    ...
    "name": "南看台封階",
    "zone": "arena_quarter",
    "description": "南看台封階停在一排鐵柵前，北側可望見下注牌與沙地邊線，南面階梯被維修木板完全擋住。這格保留為看台高度差的地圖 封閉點，不提供路線或隱藏互動。南看台封階周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"arena_quarter_fill_25_12": {
    ...
    "name": "南牆旗影巷",
    "zone": "arena_quarter",
    "description": "南牆旗影巷靠近競技場外牆，北側旗桿影子落在石地上，南面是封死的維修門與堆疊護欄。這裡只標示看台背後的封閉邊界，旅人必須折回北側正式通道，不會通往新區域。南牆旗影巷周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"arena_quarter_fill_26_12": {
    ...
    "name": "南練兵封欄",
    "zone": "arena_quarter",
    "description": "南練兵封欄貼著訓練場沙坑最外圈，北面傳來木劍敲擊聲，南側維修道路落差被粗繩和拒馬隔斷。這格是訓練區封閉邊界 封閉點，提醒玩家不要把它當成可通行主路，也不安排怪物或補給。南練兵封欄周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"arena_quarter_fill_26_6": {
    ...
    "name": "北側沙欄角",
    "zone": "arena_quarter",
    "description": "北側沙欄角位在決鬥場外緣，南面能看到裁判旗架，西側是武器檢查走廊人流。粗繩把觀眾與選手道路分開，這裡作為不可通行的安全緩衝格，不放怪物或服務。北側沙欄角周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"arena_quarter_fill_27_10": {
    ...
    "name": "西看台陰廊",
    "zone": "arena_quarter",
    "description": "西看台陰廊夾在下層座席與訓練沙地之間，北面可聽見看台呼喊，東側有沙地護欄。廊口被臨時布幕封住，只保留為地圖上的看台邊界與視覺過渡。西看台陰廊周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"arena_quarter_fill_27_12": {
    ...
    "name": "南看台鎖門",
    "zone": "arena_quarter",
    "description": "南看台鎖門位於座席背面，北側可回望階梯與下注牌，南側厚門掛著暫停通行的銅牌。這裡是看台外圈的封閉 封閉點，讓地圖保留建築輪廓但不開放玩家穿越，也沒有服務用途。南看台鎖門周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"arena_quarter_fill_28_12": {
    ...
    "name": "南環拒馬道",
    "zone": "arena_quarter",
    "description": "南環拒馬道沿著競技場外圈延伸，北面有選手休息棚的燈火，南側一整排拒馬封住維修坡。這格不是主路，而是用來阻斷南側空白區的安全邊界。南環拒馬道周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"arena_quarter_fill_28_8": {
    ...
    "name": "中層看台欄",
    "zone": "arena_quarter",
    "description": "中層看台欄位於座席轉角，西側通向觀眾席走廊輪廓，南面能俯看決鬥沙地。欄杆後方只剩狹窄階梯落差與維修架，這裡標示封閉看台，不提供通行路線。中層看台欄周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"arena_quarter_fill_29_8": {
    ...
    "name": "東看台轉角",
    "zone": "arena_quarter",
    "description": "東看台轉角被冠軍畫像與繩欄包圍，西側是觀眾席的回聲，北面能聽見票券柱廊的人潮。後方維修梯已收起，因此這格只作為看台外輪廓與不可通行邊界。東看台轉角周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"arena_quarter_fill_30_10": {
    ...
    "name": "東門封鎖線",
    "zone": "arena_quarter",
    "description": "東門封鎖線位於競技城區外緣，西側能回望沙地與裁判席，東面大門被修繕木架擋住。這裡是東側地圖封閉 封閉點，讓玩家明確知道要回主場內側尋路，避免誤入城外空格。東門封鎖線周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"arena_quarter_fill_30_11": {
    ...
    "name": "東側石階封口",
    "zone": "arena_quarter",
    "description": "東側石階封口貼著看台背牆，西面可見座席階梯，東側石階被崩落碎塊堵住。它保留建築高度差與邊界感，不放服務或怪物，玩家需要折回西側通道。東側石階封口周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"arena_quarter_fill_30_7": {
    ...
    "name": "北東旗欄",
    "zone": "arena_quarter",
    "description": "北東旗欄插滿決鬥旗幟，西側接近武器檢查線，南面能看見觀眾入口的石階。東側外牆沒有開門，這格作為北東角封閉 封閉點，避免地圖誤導玩家走出場區。北東旗欄周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"arena_quarter_fill_30_8": {
    ...
    "name": "東看台欄門",
    "zone": "arena_quarter",
    "description": "東看台欄門面向上層座席，西側通往觀眾席輪廓，東面欄門被鎖鏈固定。這裡是看台外圈的封閉 封閉點 節點，只提供地圖輪廓與路線警示，不是服務房，也不放任務目標。東看台欄門周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"arena_quarter_fill_30_9": {
    ...
    "name": "東外牆窄巷",
    "zone": "arena_quarter",
    "description": "東外牆窄巷沿競技場石牆排列，西側能聽見場內歡呼，東側牆根堆滿封存拒馬與破旗架。這裡作為不可通行的封閉邊界，玩家只能回到西側正式競技區路線，不會接出新通道。東外牆窄巷周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"blackwood_fill_n11_15": {
    ...
    "name": "空心木橋東枯枝封",
    "zone": "blackwood",
    "description": "空心木橋東枯枝封貼著黑木林深處邊界，西側倒木橋腹仍傳來空洞回音，北面長老樹環的黑根垂下階梯。枯枝棚架封住東南側錯路，樹皮可刮取少量黑木炭皮，但這裡主要是 邊界封閉點。空心木橋東枯枝封周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"blackwood_fill_n12_13": {
    ...
    "name": "倒祠北苔根採封",
    "zone": "blackwood",
    "description": "倒祠北苔根採封位在倒塌小祠北側，南面的斷柱被黑苔吞住，東邊枯枝棚縫透出灰光，西側還能聽見骨鈴晃動。石板下盤根錯結，可採黑苔與夜蕨，但根牆阻斷深入路線。倒祠北苔根採封周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"blackwood_fill_n13_13": {
    ...
    "name": "骨鈴東枯根採封",
    "zone": "blackwood",
    "description": "骨鈴東枯根採封夾在骨鈴林與倒祠北苔根之間，西面的骨片風鈴敲出空聲，南側黑樹脂池滲出暗亮黏液，北邊影狼窩留下爪痕。枯根可採夜蕨毒液，卻也是封住狼徑外緣的 封閉點。骨鈴東枯根採封周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"blackwood_fill_n15_12": {
    ...
    "name": "女巫樹洞北炭根封",
    "zone": "blackwood",
    "description": "女巫樹洞北炭根封在黑木林西側高根上，南面女巫樹洞掛著綠火，東側無月空地吞掉所有月光。炭黑樹根裸露成硬脊，可刮黑木炭皮補給，但北西兩面被根牆封死，只作 封閉點。女巫樹洞北炭根封周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"blackwood_fill_n17_13": {
    ...
    "name": "炭門北黑苔封徑",
    "zone": "blackwood",
    "description": "炭門北黑苔封徑守在炭樹入口北面，南側焦黑門柱仍有獵人刻痕，東邊黑苔床散出濕冷毒味。地面積著可採的夜蕨與黑苔，但西側樹影反覆閉合，是提醒玩家回到主路的 封閉點。炭門北黑苔封徑周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"bloodsalt_coast_fill_40_7": {
    ...
    "description": "潮門南赤崖棧道是血鹽海岸北段的正式路線 路線，北面潮門入口仍有霧港鹽霧，南側鹵蝕小徑切進紅色海崖，東邊骨網淺灘掛著破魚骨。鏽木板雖晃動，但仍標出南北通行線。潮門南赤崖棧道周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"bloodsalt_coast_fill_40_9": {
    ...
    "name": "鹵蝕南側紅鹽封礁",
    "zone": "bloodsalt_coast",
    "description": "鹵蝕南側紅鹽封礁位在鹵蝕小徑南面，北側切痕仍積著血紅鹽水，東邊淹沒瞭望塔露出斷梁。紅鹽晶把礁石黏成尖刺，是純 封閉點，阻止玩家從小徑背面切向瞭望塔。鹵蝕南側紅鹽封礁周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"bloodsalt_coast_fill_41_10": {
    ...
    "name": "淹塔南鏽棧封",
    "zone": "bloodsalt_coast",
    "description": "淹塔南鏽棧封貼在淹沒瞭望塔南側，北面塔腳浸在紅潮裡，東側戰旗沙丘的破布在鹽風中甩動。棧道被腐蝕鐵釘與斷板封住，是 封閉點 邊界，不開通往沙丘背面的捷徑。淹塔南鏽棧封周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"bloodsalt_coast_fill_43_6": {
    ...
    "name": "破船標東赤鹽崖",
    "zone": "bloodsalt_coast",
    "description": "破船標東赤鹽崖位在破船標東面，西側殘骸木牌被紅鹽包住，南方猩紅潮池不斷冒泡。礁崖覆滿鋒利鹽晶，是純 封閉點，用來封住破船標與潮池之間的危險斜面。破船標東赤鹽崖周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"bloodsalt_coast_fill_44_7": {
    ...
    "name": "猩紅潮池東鏽崖",
    "zone": "bloodsalt_coast",
    "description": "猩紅潮池東鏽崖壓在潮池東側，西面血色池水拍打礁壁，南方走私者海灣藏在低霧裡。崖邊棧道鏽斷成數截，裂縫裡凝著暗紅鹽晶與潮池血苔，是 封閉點 地形，阻止玩家從潮池直接滑入海灣背路。猩紅潮池東鏽崖周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"bloodsalt_coast_fill_45_9": {
    ...
    "name": "血稅碼頭北礁封",
    "zone": "bloodsalt_coast",
    "description": "血稅碼頭北礁封位在血稅碼頭北面，南側木樁掛著暗紅繩結，西邊冰暗湧潮把黑水推上礁面。紅鹽晶與碎骨網封住通道，是純 封閉點，標示碼頭北側不可繞行。血稅碼頭北礁封周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"bloodsalt_coast_fill_47_9": {
    ...
    "name": "深海神殿西沉門路牌亭",
    "zone": "bloodsalt_coast",
    "description": "深海神殿西沉門路牌亭位在血鹽海岸最東端，西面斷棧只剩鹽蝕木樁，東側半沉石門刻滿古文，門後透出藍色火光。路牌亭標示這裡是跨區邊界路線端點，專門銜接血鹽海岸與深海神殿潮門，不放一般遭遇。深海神殿西沉門路牌亭周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"dark_forest_fill_n1_13": {
    ...
    "name": "鐵木影門苔徑",
    "zone": "dark_forest",
    "description": "鐵木影門苔徑貼著暗影森林東緣，西側樹根覆住黑苔石，東面能看見鐵木要塞傳送院的冷光。這裡是跨區 邊界 路線與採集邊界，苔蘚裡有濕木藥草痕跡，腳下石縫很窄。鐵木影門苔徑周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"dark_forest_fill_n1_14": {
    ...
    "name": "軍需陰根道",
    "zone": "dark_forest",
    "description": "軍需陰根道被粗樹根壓成窄徑，西側黑葉遮住回頭路，東面要塞軍需排屋的木牆從枝縫露出。這裡是暗林與鐵木要塞的 邊界 路線，樹根旁留有採集痕跡與舊巡邏刻痕。軍需陰根道周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"dark_forest_fill_n1_15": {
    ...
    "name": "熔爐煙苔路",
    "zone": "dark_forest",
    "description": "熔爐煙苔路藏在黑樹與濕苔之間，西側暗林仍有冷霧，東面鐵木要塞鍛造區的煙味穿過枝葉。這裡是跨區 邊界 通道，苔石上可見被熱灰壓暗的草藥痕與焦黑足印，路面狹窄。熔爐煙苔路周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"dark_forest_fill_n1_16": {
    ...
    "name": "鐵木根牆徑",
    "zone": "dark_forest",
    "description": "鐵木根牆徑沿著暗林東側延伸，西面黑樹根纏成牆，東側鐵木要塞林苑透出樹脂火光。這裡是 邊界 路線與採集邊界，玩家需沿東側根牆出口進入要塞林苑，不可往南硬穿。鐵木根牆徑周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"dark_forest_fill_n2_14": {
    ...
    "name": "北苔根封口",
    "zone": "dark_forest",
    "description": "北苔根封口被古樹根盤成低牆，東側能聽見軍需陰根道的枝葉摩擦，南面暗林更深。苔根旁有採集痕跡，但根牆完全封住路線，這格是封閉 封閉點。北苔根封口周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"dark_forest_fill_n3_14": {
    ...
    "name": "中段腐根欄",
    "zone": "dark_forest",
    "description": "中段腐根欄位於暗影森林腹地，東側北苔根封口仍可望見，西面枯枝把地面壓得鬆軟。腐根縫裡有黑苔和小菌株，這裡是採集邊界，不提供穿越路線。中段腐根欄周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"dark_forest_fill_n4_12": {
    ...
    "name": "北影苔石坪",
    "zone": "dark_forest",
    "description": "北影苔石坪位在暗林北側高處，南面可聽見盤根橋附近的濕木聲，東側苔石被黑葉覆住。這裡只有苔蘚與草藥採集痕跡，是封閉 封閉點，不接出北側新路。北影苔石坪周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"dark_forest_fill_n4_14": {
    ...
    "name": "內林苔階封口",
    "zone": "dark_forest",
    "description": "內林苔階封口由幾塊濕滑石階堆成，北側腐根欄壓住舊路，南面暗林霧氣更重。石階縫裡能採到黑苔，但前方根牆坍住，這格只作封閉 封閉點 邊界。內林苔階封口周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"dark_forest_fill_n4_16": {
    ...
    "name": "南影根幕",
    "zone": "dark_forest",
    "description": "南影根幕垂滿黑色細根，北面內林苔階封口仍有微弱反光，東側鐵木根牆徑被枝影遮住。根幕下有採集痕跡，但無法通行，是暗林內圈 封閉點 邊界。南影根幕周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"dark_forest_fill_n4_17": {
    ...
    "name": "南腐葉封溝",
    "zone": "dark_forest",
    "description": "南腐葉封溝是一條積滿黑水與腐葉的林下溝道，北側南影根幕擋住視線，東面能聞到濕木與要塞煙味。溝邊斷木橋只剩兩根濕滑木梁，梁下有藥草採集痕跡，但沒有穩固踏點，這格是封閉 封閉點。南腐葉封溝周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"dark_forest_fill_n4_18": {
    ...
    "name": "南界苔石斷階",
    "zone": "dark_forest",
    "description": "南界苔石斷階位於暗影森林南緣，北側腐葉封溝逐漸收窄，南面石階被坍根和黑苔覆住。這裡是 邊界封閉點，保留採集痕跡但禁止往南硬穿，斷階下沒有落腳點或路標，只能折回北側。南界苔石斷階周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"dark_forest_fill_n7_16": {
    ...
    "name": "西側黑藤欄",
    "zone": "dark_forest",
    "description": "西側黑藤欄靠近暗林與黑木林邊界，東面能回望腐葉封溝的霧氣，西側藤牆下有濕亮草葉。這裡是採集邊界與封閉 封閉點，藤牆後方沒有安全路標。西側黑藤欄周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"eastern_coast_fill_31_0": {
    ...
    "name": "北吊機礁封路",
    "zone": "eastern_coast",
    "description": "北吊機礁封路貼著東海岸北端，北面能望見機械墳場入口吊機的黑影，南側海藻灘在潮聲中展開，東邊木棧道只剩遠處欄杆。這裡是 邊界封閉點，亂礁與廢纜封住機械區方向，不提供正式通行。北吊機礁封路周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"eastern_coast_fill_31_3": {
    ...
    "name": "王道東側潮封",
    "zone": "eastern_coast",
    "description": "王道東側潮封位在王道市集東界外，西面可回望市集石路的旗桿，北側潮池洞窟滲出鹹水，東邊海蝕洞口被浪花遮住。濕滑礁面上立著潮門木牌、破繩欄與白色浪痕，明確標示市集與東海岸分界；這裡是 邊界封閉點，不作跨區捷徑。王道東側潮封周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"eastern_coast_fill_34_4": {
    ...
    "name": "海盜營南沙封",
    "zone": "eastern_coast",
    "description": "海盜營南沙封位在海盜營地南側，北面破帆與煙灰仍可辨認，西側暗礁貼著退潮線，南面潮間帶積著碎貝。這裡是 封閉點 邊界，鬆沙下埋著木樁與斷索，只用來阻止玩家繞過營地路線。海盜營南沙封周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"eastern_coast_fill_34_5": {
    ...
    "name": "藍泥北側潮溝",
    "zone": "eastern_coast",
    "description": "藍泥北側潮溝夾在海盜營南沙封與鹽風灘藍泥棚之間，北面乾沙逐漸變成礁石，南側藍黑色濕泥開始滲出鹽霧。潮溝水深不穩，是純 封閉點 與過渡地形提示，不安排採集或戰鬥。藍泥北側潮溝周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"eastern_coast_fill_34_6": {
    ...
    "name": "鹽風藍泥北界",
    "zone": "eastern_coast",
    "description": "鹽風藍泥北界位在東海岸南緣，北面潮溝仍有海浪回聲，南側鹽風灘藍泥棚泛著藍黑濕光。這裡是跨區 邊界 路線 端點，標示海岸沙礁轉入鹽風灘濕泥，不放怪物或採集玩法。鹽風藍泥北界周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"eastern_coast_fill_35_0": {
    ...
    "name": "斷碼頭東礁封",
    "zone": "eastern_coast",
    "description": "斷碼頭東礁封位在破碼頭東側，西面木樁被浪打得歪斜，南方走私者海灣藏在礁影後。礁石裂縫被海水灌滿，這裡是 邊界封閉點，阻止碼頭線直接切入海灣背面。斷碼頭東礁封周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"eastern_coast_fill_36_2": {
    ...
    "name": "蛇巢東側浪封",
    "zone": "eastern_coast",
    "description": "蛇巢東側浪封貼著海蛇巢穴東面，西側礁洞裡有蛇鱗刮痕，北方風暴瞭望崖被浪霧遮住，東邊低礁延伸成一串斷點。這裡是 封閉點 邊界，濕滑浪溝阻止巢穴路線向東亂接。蛇巢東側浪封周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"eastern_coast_fill_37_2": {
    ...
    "name": "中段碎礁阻帶",
    "zone": "eastern_coast",
    "description": "中段碎礁阻帶位在蛇巢東側浪封與霧港西潮帶之間，西面浪溝仍可聽見蛇巢回聲，東側礁脊逐漸被海霧吞沒。這裡是純 封閉點，碎礁只提供地形連續感，不是可通行路線。中段碎礁阻帶周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"eastern_coast_fill_38_2": {
    ...
    "name": "霧港西潮前帶",
    "zone": "eastern_coast",
    "description": "霧港西潮前帶承接中段碎礁阻帶，西面是低矮礁脊，東面濃霧裡已能聽見霧港銅鐘。潮水在腳下留下連續水窪，這裡是純 封閉點 與邊界預告，不安排額外玩法。霧港西潮前帶周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"eastern_coast_fill_39_2": {
    ...
    "name": "霧港西門路牌潮徑",
    "zone": "eastern_coast",
    "description": "霧港西門路牌潮徑位在東海岸最東端，西面霧港西潮前帶仍有礁水聲，東面濃霧中露出霧港城門拱影與銅鐘輪廓。路牌亭標示這裡是跨區 邊界 路線 端點，專門銜接東海岸與霧港入口，不放怪物或採集玩法。霧港西門路牌潮徑周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"ember_march_fill_22_21": {
    ...
    "name": "星坑東燼封道",
    "zone": "ember_march",
    "description": "星坑東燼封道位於餘燼邊境西北角，西面星落 crater 邊緣的暗光被灰風吞沒，南側焦炭道路還有車轍，東面火草平地冒出紅芽。焦黑道路被熔渣封住，是 邊界封閉點，不開跨區捷徑，只作邊界提示。星坑東燼封道周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"ember_march_fill_22_24": {
    ...
    "name": "南燼臺北封灰道",
    "zone": "ember_march",
    "description": "南燼臺北封灰道貼著南燼傳送石臺北側，北面焦黑里程碑半埋在玄武岩道路旁，南面傳送符文忽明忽暗，西側玻灰原吹來細碎燼沙。這裡是 邊界封閉點，標示傳送點外圍但不開放直穿。南燼臺北封灰道周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"ember_march_south_portal": {
    ...
    "name": "南燼傳送石臺",
    "zone": "ember_march",
    "description": "南燼傳送石臺以黑色玄武岩砌在焦灰南坡，北側封灰道隔開熱風，東面補給棚掛著耐火布，南面避風亭擋住熔灰。這裡是 portal service 路線，提供餘燼邊境南部傳送與補給動線。南燼傳送石臺周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"ember_march_south_supply": {
    ...
    "name": "焦土補給棚",
    "zone": "ember_march",
    "description": "焦土補給棚半埋在灰堆裡，西側可回到南燼傳送石臺，棚內耐火布擋住熱風，水桶、冷卻藥劑與備用繃帶沿牆排開。這裡是 service room，只提供南部據點補給提示，不放敵人。焦土補給棚周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
