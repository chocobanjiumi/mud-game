...
"ember_march_south_shelter": {
    ...
    "name": "熔灰避風亭",
    "zone": "ember_march",
    "description": "熔灰避風亭以低矮石牆圍住南側熱風，北面玄武岩臺階通回傳送石臺，亭外能看見火山前緣紅光。亭內有水缸、灰斗與臨時坐凳，是 service room，讓玩家辨認南部安全歇腳點。熔灰避風亭周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"ember_march_fill_26_21": {
    ...
    "name": "骨窯北封火痕",
    "zone": "ember_march",
    "description": "骨窯北封火痕位於骨窯隘道北側，南面窯口傳來乾骨裂響，西側焦灼綠洲的黑水氣味被熱風推來。玄武岩道路上的火痕斷成數截，熔灰覆住踏點，是 邊界封閉點，不提供穿越。骨窯北封火痕周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"ember_march_fill_28_22": {
    ...
    "name": "龍印東封灰脊",
    "zone": "ember_march",
    "description": "龍印東封灰脊位於龍印山脊東面，西側巨大爪痕仍壓在焦土裡，南面邊境要塞殼露出黑石牆角。灰脊盡頭被熱風削成斷坡，這裡是 邊界封閉點，只標示火山前緣方向。龍印東封灰脊周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"ember_march_fill_30_21": {
    ...
    "name": "火山灰田西界",
    "zone": "ember_march",
    "description": "火山灰田西界位於餘燼邊境東北角，東面火山灰田的黑灰被熱風捲起，西側燼地石道已被熔渣封住。這裡是跨區 邊界 路線，只允許向東進入火山地帶，內側灰坡不可回切。火山灰田西界周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"ember_march_fill_30_23": {
    ...
    "name": "硫泉西界燼路",
    "zone": "ember_march",
    "description": "硫泉西界燼路貼著心火裂口東側，西面餘燼仍有紅光，南側灰燼邊道沉入熱泥，東面火山硫磺泉噴出刺鼻白霧。路邊硫磺晶簇和灰燼礦露在熱泥旁，可沿石縫採集；這裡只導向火山溫泉前緣，不開旁支道路。硫泉西界燼路周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"ember_march_fill_30_24": {
    ...
    "name": "熔橋西界灰道",
    "zone": "ember_march",
    "description": "熔橋西界灰道位於餘燼邊境東南口，北側硫泉西界仍有白霧，東面火山熔岩橋發出紅光。灰道路基被熱浪切開，這裡是跨區 邊界 路線，只讓玩家向東接上熔岩橋，不開南側灰坡。熔橋西界灰道周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"emerald_canopy_fill_n18_7": {
    ...
    "name": "琥珀接枝平台",
    "zone": "emerald_canopy",
    "description": "琥珀接枝平台架在翡翠樹冠東緣，西側是高枝走道的綠蔭，東面樹脂光線銜接琥珀森林樹門。這裡是跨區過渡路線，枝幹上有可採集的嫩葉與樹脂痕，提醒玩家已離開單純樹冠區。琥珀接枝平台周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"emerald_canopy_fill_n19_7": {
    ...
    "name": "北冠斷枝欄",
    "zone": "emerald_canopy",
    "description": "北冠斷枝欄位於高空枝道邊緣，東側能看見琥珀接枝平台，南面厚葉遮住下層樹影。幾根斷枝被藤索綁成封閉 封閉點，枝縫間有露珠草葉可採，但不能離開正式樹冠路線。北冠斷枝欄周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"emerald_canopy_fill_n20_7": {
    ...
    "name": "霧葉封索道",
    "zone": "emerald_canopy",
    "description": "霧葉封索道被濕藤與寬葉覆住，東側連著北冠斷枝欄，西面樹冠逐漸抬高。藤索上掛著採集標記，表示這裡只作為資源邊界與封閉 封閉點，玩家需要折回主枝道。霧葉封索道周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"emerald_canopy_fill_n21_10": {
    ...
    "name": "南露藤欄",
    "zone": "emerald_canopy",
    "description": "南露藤欄懸在樹冠南側，北面有雨水順著粗枝滴落，西側可望見更高的綠心枝影。欄外只剩潮濕空隙與垂藤，這格標示不可通行邊界，同時保留少量露葉採集線索。南露藤欄周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"emerald_canopy_fill_n21_7": {
    ...
    "name": "北風枝籬",
    "zone": "emerald_canopy",
    "description": "北風枝籬沿著樹冠北緣延伸，東側是霧葉封索道，南面可聽見鳥民棲枝的回聲。交錯樹枝形成封閉 封閉點，枝葉間仍有藥草與花粉痕，但沒有可走的下層路。北風枝籬周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"emerald_canopy_fill_n22_7": {
    ...
    "name": "翡翠苔枝口",
    "zone": "emerald_canopy",
    "description": "翡翠苔枝口鋪滿厚苔與亮葉，東側可回北風枝籬，西面枝幹被鳥巢與藤蔓封住。這裡是資源採集邊界，不放正式出口，玩家只能沿主樹冠路線尋找安全通行點。翡翠苔枝口周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"emerald_canopy_fill_n22_8": {
    ...
    "name": "中層葉棚封口",
    "zone": "emerald_canopy",
    "description": "中層葉棚封口位於北冠下方，北側枝影透出翠綠光線，東面有細藤垂到不可攀的深谷。這格是高低差 封閉點，葉棚上可看見採集痕跡，但不提供通往下層的路。中層葉棚封口周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"emerald_canopy_fill_n23_7": {
    ...
    "name": "西冠鳥巢欄",
    "zone": "emerald_canopy",
    "description": "西冠鳥巢欄靠近樹冠西緣，東側接著翡翠苔枝口，南面傳來高枝鳥鳴。鳥巢與護藤把外側完全封住，這裡只作為採集與邊界提示，不能穿出翡翠樹冠。西冠鳥巢欄周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"emerald_canopy_fill_n25_11": {
    ...
    "name": "西南空枝臺",
    "zone": "emerald_canopy",
    "description": "西南空枝臺伸向樹冠邊界，北側高枝逐漸稀疏，東面可望見雨水落入下層葉海。外側沒有棧板，藤索也被收起，這格是封閉 邊界封閉點，只保留採集葉痕與高度警示，不安排路線出口。西南空枝臺周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"emerald_canopy_fill_n26_10": {
    ...
    "name": "西界風孔枝",
    "zone": "emerald_canopy",
    "description": "西界風孔枝貼著翡翠樹冠最外圈，南側風洞把葉片吹得翻白，東面能看見安全主枝道。這裡是西側封閉邊界，枝面濕滑且只留下採集標記，不應作為通行路線，也沒有下層踏點。西界風孔枝周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"emerald_canopy_fill_n26_8": {
    ...
    "name": "西北葉幕欄",
    "zone": "emerald_canopy",
    "description": "西北葉幕欄被大片亮葉遮成綠色牆面，南側可望見西界風孔枝，東面高處傳來鳥民哨音。葉幕後方是空落枝縫，這格作為封閉 邊界封閉點，提醒玩家回到內側路線，避免誤入斷枝。西北葉幕欄周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"frostbite_pass_fill_n19_n12": {
    ...
    "name": "雨棚西南冰封坡",
    "zone": "frostbite_pass",
    "description": "雨棚西南冰封坡位在風暴高原雨棚西南側，北面東界凍石小路滴著冰水，西側冰封橫道被硬雪堵住。坡面結成薄冰，是純 封閉點，避免高原邊界向南外擴。雨棚西南冰封坡周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"frostbite_pass_fill_n19_n13": {
    ...
    "name": "雨棚西界凍石路牌",
    "zone": "frostbite_pass",
    "description": "雨棚西界凍石路牌位在霜咬隘口東側，北面崖門西界繼續抬升，南側薄冰坡被冷雨封住，東邊風暴高原雨棚有濕暖氣流。路牌標示這裡是跨區邊界路線端點，專門銜接隘口與高原雨棚。雨棚西界凍石路牌周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"frostbite_pass_fill_n19_n14": {
    ...
    "name": "高原崖門西凍路牌",
    "zone": "frostbite_pass",
    "description": "高原崖門西凍路牌位在霜咬隘口東北界，北面惡魔邊境冷影壓在山脊上，南側雨棚西界凍石路繼續下滑，東邊風暴高原崖門透出濕暖風。這裡是跨區邊界路線端點，只銜接隘口與高原崖門。高原崖門西凍路牌周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"frostbite_pass_fill_n20_n12": {
    ...
    "name": "東段冰封橫道",
    "zone": "frostbite_pass",
    "description": "東段冰封橫道夾在雨棚西南冰封坡與極封門南側之間，東面薄冰坡反光刺眼，西面霜咬窄道被硬雪壓低。冰面裂縫密集，是純 封閉點，不作正式通行路線。東段冰封橫道周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"frostbite_pass_fill_n21_n12": {
    ...
    "name": "極封門南霜脊",
    "zone": "frostbite_pass",
    "description": "極封門南霜脊位在極封門南面，北側門柱覆滿藍霜，西邊北行山脊露出風削石階，東側冰封橫道反著白光。這裡是 封閉點，用霜脊與倒冰柱封住極封門背面的外側空地。極封門南霜脊周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"frostbite_pass_fill_n22_n14": {
    ...
    "name": "龍息台東冰裂牆",
    "zone": "frostbite_pass",
    "description": "龍息台東冰裂牆位在龍息岩棚北側偏東，南面岩棚有焦黑霜痕，西邊雨雪哨的旗索在風裡繃緊。冰牆裂縫灌出刺骨冷風，是純 封閉點，封住岩棚外側不穩定斜面。龍息台東冰裂牆周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
...
"frostbite_pass_fill_n27_n14": {
    ...
    "name": "空山風門東霜牆",
    "zone": "frostbite_pass",
    "description": "空山風門東霜牆位在霜咬隘口西北界，西面空山入口風門發出低鳴，北側雪門路牌被冰霜覆住，東邊凍折返道消失在白霧裡。這裡是 邊界封閉點，霜牆標示空山與隘口分界，不開跨區路。空山風門東霜牆周邊的路面、植被、舊標記與相鄰地貌都需要被看清，北南東西的可行方向、封閉邊線與回程線索會在場景中自然呈現，讓隊伍能判斷是否採集、折返或接回主路。",
