...
const DESCRIPTION_OVERRIDES: Record<string, string> = {
  kingdom_frontier_fill_n10_n7: '號角獵門口穿過王室獵場西側界碑，西面邊境巡邏路仍有軍靴印，東側獵場號角門掛著舊銅環。磨損石墩標出獵場與軍管區分界，是明確跨區通路；兩側都有巡邏繩標、放行牌與獵角警示。門下石板刻著王室獵令。',
  kingdom_frontier_fill_n10_n6: '邊境巡邏路在獵場鹿徑西側延伸，西面是軍方哨旗與低牆，東側草叢裡可見王室與軍方的雙重界標。這是通往王家獵場鹿徑的 border route，路面仍受巡邏管制；界碑旁有警告牌、蹄印與巡邏繩標。玩家可辨認唯一放行缺口。',
  ...
  bloodsalt_coast_fill_47_9: '深海神殿西沉門路牌亭位在血鹽海岸最東端，西面斷棧只剩鹽蝕木樁，東側半沉石門刻滿古文，門後透出藍色火光。路牌亭標示這裡是跨區邊界路線端點，專門銜接血鹽海岸與深海神殿潮門，不放一般遭遇。',
  ...
  old_farmland_fill_n1_1: '倉庫後穀車道貼著舊農田東界，東側是新手村倉庫斑駁後門，西面田埂上還留著壓碎的穀車轍。這裡是 border service route，標示農田補給與村莊倉儲的銜接，只供後勤往來，不放怪物遭遇。',
  old_farmland_fill_n1_2: '水井旁廢農道位於舊農田東側，北面可回望倉庫後穀車道，南面雜草沒入河階方向，東側村莊水井旁有石圈與木桶。這裡是 border service route，作為農田取水與補給服務邊界，只連接村內取水點。',
  // 新手村外圍（地窖附近）
