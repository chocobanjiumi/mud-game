...
function buildSystemErrorAuditRecords(): { id: string; text: string; numeric?: boolean }[] {
  return [
    ...
    { id: 'npcDialogue/entryLevelDisabled', text: '你正在查看副本入口「水晶洞窟」，但等級不足；目前等級 3，需求等級 5。下一步請先完成同等級任務或提升等級後再返回入口。', numeric: true },
    { id: 'npcDialogue/entryPartyDisabled', text: '你正在查看副本入口「水晶洞窟」，但隊伍人數不符；目前 6 人，最多 5 人。下一步請調整隊伍人數後由隊長再次進入。', numeric: true },
    { id: 'npcDialogue/entryItemDisabled', text: '你正在嘗試進入「終焉戰場軍旗裂隙」，但入口道具不足；缺少道具「終戰軍旗封印」，目前持有 0 個，需求 1 個。下一步：取得所需道具後回到此入口。', numeric: true },
    { id: 'npcDialogue/entryQuestDisabled', text: '你正在嘗試進入「王城封印門」，但任務條件不足；需要「王都封印調查」達到「已完成」，目前狀態是「進行中」。下一步：先推進對應任務階段再返回入口。' },
    { id: 'npcDialogue/entryCooldownDisabled', text: '你正在查看副本入口「終焉戰場」，但入口仍在冷卻中；目前剩餘 60 秒，需求剩餘 0 秒後才能再次開啟。下一步請等待冷卻結束後再使用入口。', numeric: true },
