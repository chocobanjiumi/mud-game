...
const advancedClassPlans: AdvancedClassPlan[] = [
  ...
  },
  {
    ...
    signatureSkills: [
      '烈焰爆破：高 MP 消耗，指定房間小範圍火傷，燃燒會在怪物 approaching 時持續跳傷。',
      '冰封地帶：指定方向相鄰房範圍冰傷，命中怪物 arrivalTicks +1。',
      '連鎖閃電：本房或相鄰房跳躍多目標，對濕潤/金屬怪物效果提高。',
      '元素共鳴：短時間內不同元素連續命中時返還部分 MP。',
      '隕星雨：四方相鄰房高風險 AoE，傷害高但會讓所有存活怪進入 approaching。',
    ],
  },
  {
    id: 'arcanist',
    ...
    signatureSkills: [
      '奧術屏障：消耗 MP，給自己或隊友護盾，吸收傷害並降低打斷。',
      '魔力汲取：對目標造成低傷並回復 MP，對施法怪效果更好。',
      '定錨結界：指定出口設置結界，抵達怪物命中下降或 arrivalTicks +1。',
      '法術穩定：下一個詠唱技能不會被小傷害打斷，並降低 MP 消耗。',
      '奧術超載：短時間提高法術效果，但結束後進入 MP 回復下降狀態。',
    ],
  },
  {
    id: 'chronomancer',
    ...
    signatureSkills: [
      '遲滯術：指定目標 slow，若目標正在 approaching 則 arrivalTicks +1。',
      '時間鎖：打斷預兆並凍結目標 1 tick，對 Boss 改為延後預兆。',
      '回溯步：把自己或隊友回復到上一 tick 的位置/部分 HP 狀態。',
      '加速咒：隊友下一個技能 CD -1 或行動優先度提高。',
      '停滯領域：指定出口短時間停滯，所有從該出口抵達的怪物延後 1 tick。',
    ],
  },
  {
    id: 'bishop',
    ...
    signatureSkills: [
      '大治癒：信仰 +25，強力單體治療；信仰過高時不能使用。',
      '聖域護幕：全隊護盾，持續數 tick，適合怪潮抵達前施放。',
      '復甦禱言：戰鬥中復活倒下隊友，長 CD 且大幅推向慈悲端。',
      '光明祝禱：提高隊友受到治療效果，並降低下一次傷害。',
      '審判微光：低傷害光攻擊，用來把信仰從慈悲端拉回中間。',
    ],
  },
  {
    id: 'inquisitor',
    ...
    signatureSkills: [
      '審判錘：信仰 -18，光屬性重擊，對暗/undead 額外傷害。',
      '沉默敕令：打斷或沉默施法怪，成功後短暫提高聖光傷害。',
      '異端標記：標記目標，隊友對其光傷與暴擊提高。',
      '神罰：高信仰消耗的處決技，目標低血時傷害提高。',
      '懺悔治癒：小治療並把信仰往慈悲端拉回，審判者的節奏修正技。',
    ],
  },
  {
    id: 'druid',
    ...
    signatureSkills: [
      '生命再生：信仰 +12，給目標數 tick 持續治療。',
      '荊棘護甲：信仰 +10，受到近戰攻擊時反傷，適合 solo。',
      '藤蔓纏繞：信仰 -12，自然傷害並 slow；對 approaching 目標 arrivalTicks +1。',
      '野性圖騰：指定出口設圖騰，隊友經過獲得再生，怪物抵達時受 slow。',
      '翠綠奔湧：群體 HoT，長 CD，適合怪潮抵達前預鋪。',
    ],
  },
];
