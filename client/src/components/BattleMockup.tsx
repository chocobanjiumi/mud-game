        ...
        <div className="flex-1 overflow-y-auto flex flex-col">
          ...
          <div className="flex-1 min-h-[50vh] p-2 relative">
            ...
            <div className="h-full grid grid-cols-3 grid-rows-3 gap-0">
              ...
              {/* ── Row 2: 西房 | 本房 | 東房 ── */}
              <RoomCell dir="西" bg="#0a0f1a" />

              {/* 本房 (中心) */}
              <div className="relative border border-[#00ff88]/40 bg-[#00ff88]/5 rounded flex flex-col">
                {/* 本房標題 */}
                <div className="text-center py-0.5">
                  <span className="text-[9px] font-bold text-[#00ff88]">深淵龍穴 · 本房</span>
                </div>

                {/* 敵方（上半） */}
                <div className="flex-1 flex items-center justify-center gap-2 px-2">
                  <Mon img="monster_astral_time_minute_zero_dragon.png" name="深淵巨龍" hp={58} boss selected cast="龍息2t" size={48} />
                  <Mon img="monster_astral_time_oathstone_sentinel.png" name="護衛" hp={63} size={40} />
                  <Mon img="monster_astral_time_glyphbound_skeleton.png" name="法師" hp={90} cast="暗影1t" size={40} />
                </div>
                ...
                <div className="flex-1 flex flex-col justify-center px-2 gap-1">
                  ...
                  <div className="flex items-center gap-1">
                    <span className="text-[7px] text-red-400 w-4 shrink-0">前排</span>
                  ...
                  {/* 後排 */}
                  <div className="flex items-center gap-1">
                    <span className="text-[7px] text-[#00ff88] w-4 shrink-0">後排</span>
                    <div className="flex gap-2">
...
/* ═══ 房間格 ═══ */
function RoomCell({ dir, bg, children }: { dir: string; bg: string; children?: React.ReactNode }) {
  const hasContent = !!children;
  return (
    <div className={`rounded border p-1 flex flex-col ${hasContent ? 'border-[#1a3a5c] bg-[#0a0f1a]/80' : 'border-[#1a3a5c]/30 bg-[#0a0f1a]/20'}`} style={{ backgroundColor: hasContent ? undefined : bg }}>
      <div className="text-[8px] text-[#5a6a8a] text-center mb-0.5">{dir}方房間</div>
      {hasContent ? (
        <div className="flex-1 flex items-center justify-center gap-1.5 flex-wrap">
          {children}
        </div>
      ) : (
        <div className="flex-1 flex items-center justify-center text-[9px] text-[#5a6a8a]/40">安全</div>
      )}
    </div>
  );
}
...
function Mon({ img, name, hp, boss, selected, cast, size = 36 }: {
  ...
  return (
    <div className="flex flex-col items-center gap-0.5 cursor-pointer group" title={`${name} HP:${hp}%`}>
      ...
      {boss && <span className="text-[7px] text-red-400 font-bold leading-none">BOSS</span>}
    </div>
  );
}
