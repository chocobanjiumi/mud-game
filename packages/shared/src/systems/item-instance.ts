...
export const AFFIX_BUILD_DIRECTIONS: AffixBuildDirection[] = [
  ...
  { id: 'warrior_lancer', classId: 'swordsman', name: '戰士：遠程槍與 approaching 攔截', weaponCategories: ['spear'], skillTags: ['control', 'cross_room', 'interrupt'], affixIds: ['behavior_cross_room_t3', 'behavior_snare_t4'], notes: '支援戰士以雙手槍作為遠程武器，補足直線穿刺、出口槍陣與抵達攔截，定位是守路與跨房壓制。' },
  ...
  { id: 'ranger_assassin', classId: 'ranger', name: '遊俠：背刺毒與煙幕', weaponCategories: ['dagger'], skillTags: ['damage', 'mobility', 'control'], affixIds: ['behavior_swift_t3', 'behavior_execute_t5'], notes: '支援匕首影刃打剛抵達目標增傷、閃避後節奏與處決窗口，玩法目標是高機動單體爆發與短暫控制。' },
  ...
  { id: 'priest_druid', classId: 'priest', name: '祭司：HoT 與自然控制', weaponCategories: ['staff', 'holy_tome'], skillTags: ['heal', 'control', 'nature'], affixIds: ['behavior_mercy_t2', 'behavior_snare_t4'], notes: '支援法杖與聖典德魯伊 HoT、荊棘、藤蔓與出口圖騰，玩法目標是自然續補與區域控制，爆發較弱。' },
];
...
export const AFFIX_POOLS: Record<AffixPool, AffixDef[]> = {
  numeric: [
    { id: 'numeric_str_t1', name: '力量', kind: 'prefix', pool: 'numeric', tier: 'T1', appliesTo: ['weapon', 'offhand', 'hands', 'ring', 'necklace'], itemLevelMin: 1, itemLevelMax: 20, stats: { str: 1 } },
  ...
  combat: [
    ...
    { id: 'combat_def_t2', name: '守勢', kind: 'suffix', pool: 'combat', tier: 'T2', appliesTo: ['offhand', 'head', 'body', 'hands', 'feet', 'belt'], itemLevelMin: 8, itemLevelMax: 35, stats: { def: 4 } },
    ...
    { id: 'combat_hit_t2', name: '準星', kind: 'suffix', pool: 'combat', tier: 'T2', appliesTo: ['weapon', 'ring', 'earring'], itemLevelMin: 8, itemLevelMax: 35, stats: { hitRate: 3 }, weaponTypes: ['bow', 'crossbow', 'spear', 'staff', 'grimoire', 'focus'] },
  ...
  behavior: [
    ...
    { id: 'behavior_burst_t1', name: '蓄勢', kind: 'behavior', pool: 'behavior', tier: 'T1', appliesTo: ['weapon', 'ring'], itemLevelMin: 1, itemLevelMax: 24, skillTags: ['burst'], behavior: 'burst_damage', trigger: 'on_cast', skillModifiers: { damagePct: 3 }, weaponTypes: ['sword', 'blade', 'dagger', 'katana', 'giant_sword', 'axe', 'greataxe', 'hammer', 'warhammer'] },
    ...
    { id: 'behavior_counter_t4', name: '反擊', kind: 'behavior', pool: 'behavior', tier: 'T4', appliesTo: ['weapon', 'offhand', 'hands'], itemLevelMin: 32, skillTags: ['control'], behavior: 'counter_on_block', trigger: 'on_block', resourceModifiers: { rageGain: 3 } },
    { id: 'behavior_cross_room_t3', name: '鷹眼', kind: 'suffix', pool: 'behavior', tier: 'T3', appliesTo: ['weapon', 'head', 'ring'], itemLevelMin: 18, itemLevelMax: 50, skillTags: ['cross_room'], sourceTags: ['plains', 'scout', 'ranged'], behavior: 'cross_room_accuracy', trigger: 'on_hit', skillModifiers: { rangeDelta: 1 }, weaponTypes: ['bow', 'crossbow', 'spear'] },
    { id: 'behavior_snare_t4', name: '絆足', kind: 'suffix', pool: 'behavior', tier: 'T4', appliesTo: ['weapon', 'feet'], itemLevelMin: 32, skillTags: ['control'], sourceTags: ['forest', 'trap', 'gathering'], behavior: 'delay_approach', trigger: 'on_hit', condition: 'approaching_target', skillModifiers: { arrivalTicksDelta: 1 }, weaponTypes: ['bow', 'crossbow', 'spear'] },
