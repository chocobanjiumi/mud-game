...
const RAW_SKILL_DEFS: Record<string, RawSkillDef> = {
  ...
  warrior_slash: {
    ...
    targetType: 'single_enemy', resourceCost: 0, cooldown: 0,
    damageType: 'physical', element: 'none', multiplier: 1.0,
    description: '消耗 0 怒氣；本房單體攻擊，命中後獲得 8 怒氣。戰士用它穩定建立怒氣，讓後續挑釁、橫掃與防禦技能能接上節奏。',
  ...
  power_strike: {
    ...
    description: '消耗 35 怒氣；本房單體中高傷害，附加 3 tick 破甲 -12%。對已被挑釁的目標破甲效果更穩，適合在隊伍集火前先降低目標防禦。',
    shortDescription: '本房單體重擊，附加 3 tick 破甲 -12%。',
    fullDescription: '消耗 35 怒氣，冷卻 3。攻擊本房單體，造成 150% 物理傷害並附加 3 tick 破甲 -12%；對已被挑釁目標命中時額外獲得 5 怒氣。',
  ...
  blade_aura: {
    ...
    description: '消耗 25 怒氣；攻擊本房戰鬥中的最多 3 隻怪物，每命中 1 隻返還 2 怒氣。這是戰士處理多怪壓力的第一個範圍工具。',
    shortDescription: '攻擊本房最多 3 隻戰鬥怪物，每命中 1 隻返還 2 怒氣。',
    fullDescription: '消耗 25 怒氣，冷卻 3。對本房戰鬥中最多 3 隻敵人造成 80% 物理傷害；每命中 1 隻返還 2 怒氣。不影響相鄰房。',
    special: { maxTargets: 3, resourceGainPerHit: 2 },
  },
  iron_wall: {
  ...
  taunt: {
    ...
    damageType: 'pure', element: 'none', multiplier: 0,
    description: '消耗 10 怒氣；本房單體立刻進入戰鬥並優先攻擊你。若目標位於相鄰房，目標進入 approaching，arrivalTicks = 1。',
    shortDescription: '拉住本房單體；相鄰房目標會以 arrivalTicks = 1 接近。',
    fullDescription: '消耗 10 怒氣，冷卻 4。指定本房單體時，目標立刻加入戰鬥並優先攻擊你；指定相鄰房單體時，目標進入 approaching，arrivalTicks = 1。',
    effects: [{ type: 'taunt', value: 1, duration: 2 }],
    tags: ['control', 'single_target', 'resource'],
    special: { crossRoom: true, arrivalTicks: 1 },
  },
  war_cry: {
  ...
  fireball: {
    ...
    description: '消耗 15 MP；本房或指定相鄰方向單體火傷。隔房命中後目標 arrivalTicks = 2，可能驚動目標房間。',
    shortDescription: '單體火傷；隔房命中時目標 arrivalTicks = 2。',
    fullDescription: '消耗 15 MP，冷卻 1。攻擊本房或指定相鄰方向單體，造成 160% 火屬性魔法傷害；隔房命中後目標進入 approaching，arrivalTicks = 2，可能驚動同房怪物。',
    tags: ['damage', 'single_target', 'burst', 'resource', 'magical', 'fire'],
    special: { crossRoom: true, arrivalTicks: 2 },
  },
  frost_nova: {
  ...
  quick_step: {
    id: 'quick_step', name: '翻滾', englishName: 'Roll',
    classId: 'ranger', learnLevel: 1, type: 'active',
    targetType: 'self', resourceCost: 15, cooldown: 3,
    damageType: 'pure', element: 'none', multiplier: 0,
    description: '消耗 15 專注；下一 tick 閃避 +25%。若成功閃避，回復 20 專注，且下一次射擊傷害提高。',
    shortDescription: '下一 tick 閃避 +25%，成功閃避回復 20 專注。',
    fullDescription: '消耗 15 專注，冷卻 3。自己下一 tick 閃避 +25%；若成功閃避，回復 20 專注，下一次射擊傷害 +15%。',
    effects: [{ type: 'dodge_up', value: 25, duration: 1 }],
    tags: ['defense', 'mobility', 'resource'],
    special: { focusGainOnDodge: 20, nextShotDamageBonus: 15 },
  },
  poison_arrow: {
    ...
    damageType: 'pure', element: 'none', multiplier: 0,
    description: '消耗 25 專注；標記本房或已偵查相鄰房單體 4 tick。你對標記目標傷害 +15%、命中 +10%，攻擊命中時額外回復 3 專注。',
    shortDescription: '標記單體 4 tick，你對其傷害 +15%、命中 +10%。',
    fullDescription: '消耗 25 專注，冷卻 4。標記本房或已偵查相鄰房單體 4 tick；你對目標傷害 +15%、命中 +10%，攻擊標記目標命中時額外回復 3 專注。',
  ...
  ranger_scout: {
    ...
    targetType: 'self', resourceCost: 0, cooldown: 2,
    damageType: 'pure', element: 'none', multiplier: 0,
    description: '消耗 0 專注；指定方向偵查相鄰房，顯示怪物資訊並回復 15 專注。這是遊俠啟動隔房打法的核心工具。',
    shortDescription: '偵查指定方向相鄰房並回復 15 專注。',
    fullDescription: '消耗 0 專注，冷卻 2。指定方向偵查相鄰房，短時間顯示怪物資訊；使用後回復 15 專注。',
    tags: ['support', 'resource'],
    usageContext: 'both',
    special: { scoutDirection: true, resourceGain: 15 },
  },
  trap: {
  ...
  blessing: {
    id: 'blessing', name: '守護禱言', englishName: 'Guardian Prayer',
    classId: 'priest', learnLevel: 1, type: 'active',
    targetType: 'single_ally', resourceCost: 0, cooldown: 4,
    damageType: 'pure', element: 'light', multiplier: 0,
    description: '信仰 +10，條件信仰 <= 90；給自己或隊友護盾 2 tick。護盾被打破時回復少量 HP，適合承接怪物抵達前的壓力。',
    shortDescription: '信仰 +10，給自己或隊友 2 tick 護盾。',
    fullDescription: '信仰 +10，冷卻 4，條件信仰 <= 90。給自己或單一隊友護盾 2 tick；護盾被打破時回復少量 HP。',
    effects: [{ type: 'shield', value: 80, duration: 2 }],
    tags: ['support', 'defense', 'resource', 'light'],
    usageContext: 'both',
    special: { faithDelta: 10, faithMax: 90, healOnShieldBreak: true },
  },
  priest_holy_bell: {
    ...
    description: '信仰 -25，條件信仰 >= 25；東西南北相鄰房暗屬性與 undead 怪物受光傷。普通怪也會被驚動，undead 目標 arrivalTicks = 2。',
    shortDescription: '四方反邪惡光傷，undead 目標 arrivalTicks = 2。',
    fullDescription: '信仰 -25，冷卻 8，條件信仰 >= 25。影響東西南北相鄰房；暗屬性與 undead 怪物受到 100% 光屬性魔法傷害，undead 目標 arrivalTicks = 2；普通怪可能被驚動。',
    tags: ['damage', 'aoe', 'control', 'resource', 'magical', 'light'],
    special: { faithDelta: -25, faithMin: 25, crossRoom: true, areaScope: 'adjacent_cardinal', arrivalTicks: 2, undeadOnlyBonus: true },
  },
  mass_heal: {
